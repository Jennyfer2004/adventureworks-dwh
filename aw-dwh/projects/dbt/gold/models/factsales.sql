...
th as (
    select *
    from {{ source('silver', 'transactionhistory') }}
...
select 
    ...
    -- CAMBIO 3: Usamos el costo real de transaction_history en lugar del estándar del producto
    th.actualcost as standardcost, -- Renombramos la columna para mantener la consistencia
    th.actualcost * sod.orderqty  as costamount,
...
-- CAMBIO 2: Añadimos el JOIN a transaction_history
inner join th on sod.salesorderid = th.referenceorderid 
              and sod.salesorderdetailid = th.referenceorderlineid
